...
RC Table::update_record(Trx *trx, Record *old_rec, Record *new_rec) {
  ...
  bool duplicate_key = false; // Flag to track duplicate key

  // Attempt to insert new indices
  rc = insert_entry_of_indexes(new_rec->data(), new_rec->rid());

  // Handle the case of duplicate key
  if (rc == RC::RECORD_DUPLICATE_KEY) {
    duplicate_key = true;
    rc = insert_entry_of_indexes(old_rec->data(), old_rec->rid());
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to reinsert old record when updating unique has a problem");
      return rc;
    }
  ...
  if (trx != nullptr) {
    ...
    if (!duplicate_key) {
      // Append the clog record only if it's not a duplicate key
      rc = clog_manager_->clog_append_record(clog_record);
      if (rc != RC::SUCCESS) {
        return rc;
      }
