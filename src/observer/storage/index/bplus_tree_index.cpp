...
RC BplusTreeIndex::insert_entry(const char *record, const RID *rid)
{
  ...
  if (unique_ == 1) {
    ...
    if (scanner != nullptr) {
      if (is_multi_index()) {
        RID existingRid;
        while ((rc = scanner->next_entry(&existingRid)) == RC::SUCCESS) {
          Record existingRecord;
          file_handler_->get_record(&existingRid, &existingRecord);
          ...
          for (size_t i = 0; i < col_count_; i++) {
            ...
            // If any column is null, don't insert into the index
            if (is_mem_null((void *)(record + offset), attr_type, length)) {
              scanner->destroy();
              return RC::SUCCESS;
            }

            if (strncmp(record + offset, existingRecord.data() + offset, length) != 0) {
        ...
        RID unusedRid;
        rc = scanner->next_entry(&unusedRid);
