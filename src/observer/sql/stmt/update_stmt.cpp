...
RC UpdateStmt::create(Db *db, Updates &updates_sql, Stmt *&stmt)
{
  ...
  if (nullptr == table) {
    LOG_WARN("Table does not exist. db=%s, table_name=%s", db->name(), table_name);
  ...
  for (size_t pair_num = 0; pair_num < updates_sql.update_pair_num; pair_num++) {
    ...
    if (field_meta == nullptr) {
      LOG_WARN("Field not found. field=%s.%s.%s", db->name(), table->name(), attr_name);
    ...
    if (pair.is_select) {
      ...
      if (pair.select->expr_num != 1) {
        LOG_WARN("Selecting more than 1 attribute in update-select is not allowed");
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("Failed to construct filter stmt");
    return rc;
  }
